...
function ProductCard({ product }: { product: any }) {
  ...
  return (
    <div
      className='flex flex-col border shadow-sm  hover:shadow-md transition-all rounded-2xl overflow-hidden'
    ...
    >
      <Link href={`/product/${product.slug}`} className="group relative  block w-full h-60 overflow-hidden  bg-gray-100  ">
        ...
        <div className="flex flex-col items-end py-1">
          {
            product.discount > 0 ?
              <>
                <span className="font-bold text-gray-600 text-base lg:text-lg">Rs.{calculateDiscountedPrice(product.price, product.discount).discountedPrice}</span>
                <span className="text-sm text-red-500 line-through">Rs.{product.price}</span>
              </> :
              <div className="  text-base lg:text-lg capitalize">Rs.{product.price} / <span className="">
